...
import { POLAR_CLIENT_ID, POLAR_CLIENT_SECRET, POLAR_REDIRECT_URI } from '$env/static/private';
...
export async function GET({ url, cookies, fetch }) {
	...
	if (!tokenRes.ok || !tokenData.access_token || !tokenData.x_user_id) {
		return new Response('Invalid token response: ' + JSON.stringify(tokenData), { status: 500 });
	}

	// ---------------------------------------------------------------------------
	// Persist tokens + Polar user id
	// ---------------------------------------------------------------------------
	...
	// ---------------------------------------------------------------------------
	// Register user with Polar AccessLink (idempotent)
	// ---------------------------------------------------------------------------
	...
	if (![200, 201, 409].includes(registerRes.status)) {
		...
		return new Response(`User registration failed: ${registerRes.status} ${txt}`, { status: 500 });
	}
